...
//
// Deliberately NOT here: a bare "efl" -- the EFL Cup (Carabao Cup) is
// contested by Premier League clubs, so that string alone doesn't mean
// lower-division. "championship"/"league one"/"league two" already
// catch the genuine lower-tier cases; a blanket "efl" was excluding
// real top-flight news like a big club's cup exit whenever the story
// didn't happen to also name a club from BIG_FIVE below.
const SOFT_EXCLUDE = [
...
export function filterItems(items, { maxAgeHours = 24 } = {}) {
  const cutoff = Date.now() - maxAgeHours * 60 * 60 * 1000;

  return items.filter((item) => {
    if (!item.title || !item.link) return false;

    // No reliable publish date means we can't tell how stale it is --
    // treat it as stale rather than risk posting an old evergreen piece.
