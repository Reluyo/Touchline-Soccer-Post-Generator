...
export async function searchStoryImage(query) {
  ...
  // Kept deliberately terse (no per-candidate URLs, no raw response
  // dumps) -- this still answers the still-open "does search find
  // anything at all, and from which result type" question without
  // spamming a log line per candidate on every call.
  console.log(`[imageSearch] "${query}": ${imageResults.length} image results, ${results.length} web results`);
