...
export async function searchStoryImage(query) {
  ...
  console.log(`[imageSearch] Brave response keys: ${Object.keys(data).join(', ')}`);

  const results = data.web?.results || [];
  const imageResults = data.images?.results || [];

  console.log(`[imageSearch] Got ${imageResults.length} image results and ${results.length} web results for "${query}"`);

  // Log first result structure for debugging
  if (imageResults.length > 0) {
    console.log(`[imageSearch] First image result keys: ${Object.keys(imageResults[0]).join(', ')}`);
    console.log(`[imageSearch] First image result:`, JSON.stringify(imageResults[0]).slice(0, 200));
  }
  if (results.length > 0) {
    console.log(`[imageSearch] First web result keys: ${Object.keys(results[0]).join(', ')}`);
  }
  ...
  // Fallback to web results (try to use any image metadata)
  for (const result of results) {
    // Try different possible image key names that Brave might use
    const imageUrl = result.image?.url || result.thumbnail?.src || result.img;
    if (imageUrl) {
      console.log(`[imageSearch] Trying web result image: ${imageUrl.slice(0, 50)}...`);
