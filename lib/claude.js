...
{"translations":[{"index":0,"title":"...","summary":"..."}]}`;
    ...
    try {
      ...
      // Return this batch untranslated instead of failing the whole request
      batch.forEach((it, j) => {
        allTranslations.push({ index: nonEnglish.indexOf(it), title: it.title, summary: it.summary });
      });
    }
  }
  ...
  const byOriginal = new Map(
    nonEnglish.map((it) => [
      it,
      allTranslations.find((t) => t.title === it.title || t.summary === it.summary),
    ])
  );
