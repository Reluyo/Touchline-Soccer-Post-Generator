...
// {"text":"Leao",true} instead of {"text":"Leao","key":true}), leak a
// stray "=" into a property's colon (seen as "key":=true), leak a
// TypeScript type assertion onto the end of a value (seen as
// {...} as any]), or use Python's True/False/None instead of JSON's
...
export function parseJson(text) {
  const cleaned = text
    ...
    // A stray "=" wedged between a property's colon and its value --
    // seen in production as "key":=true instead of "key":true, as if a
    // JS/TS assignment briefly leaked into JSON syntax. Only strips it
    // when directly followed by something that's actually the start of
    // a JSON value, so a colon legitimately followed by "=" inside a
    // string's own text (unlikely, but not impossible) is left alone.
    .replace(/:(\s*)=(\s*)(?=true\b|false\b|null\b|"|-?\d|\[|\{)/g, ':$1$2')
    ...
    // Remove a trailing TypeScript type assertion Claude sometimes
    // appends to a value -- seen in production as {...} as any] instead
    // of {...}]. Anchored to sit directly between a value-ending token
    // (}, ], or a closing quote) and the next JSON structural character,
    // so this can't misfire on the word "as" appearing naturally inside
    // real body/headline text (e.g. "seen as a coup").
    .replace(/([}\]"])\s+as\s+[A-Za-z_$][A-Za-z0-9_$]*\s*(?=[\]},])/g, '$1')
    .trim();
  try {
...
{"ranked":[3,11,0,...]}`;
  ...
  // The model was told to return every index exactly once, but a large
  // ranked list (a busy week can be a few hundred stories) is exactly
  // the kind of long, repetitive output where it can slip and repeat an
  // index -- seen in production as the same story appearing twice in
  // the picker with two identical candidates, both toggling together
  // when clicked (they share one candidateKey) and both turning into
  // duplicate slides on build. Drop repeats as they're encountered,
  // keeping the first (best-ranked) occurrence.
  const seen = new Set();
  const order = [];
  for (const i of result.ranked || []) {
    if (stories[i] && !seen.has(i)) {
      seen.add(i);
      order.push(i);
    }
  }
...
function hasNonEmptyBody(result) {
  return typeof result.body === 'string' && result.body.trim().length > 0;
}

function hasUsableHeadlineParts(result) {
  return Array.isArray(result.headline_parts) && result.headline_parts.length > 0
    && result.headline_parts.every((p) => p && typeof p.text === 'string' && p.text.trim().length > 0);
}

// A plain, source-only fallback for when Claude just never wrote a body
// at all -- see writeSlide() below. Reuses the story's own summary
// verbatim (first sentence or two) rather than generating anything, so
// it can't invent a fact the way asking the model again risks doing a
// third time.
function fallbackBody(story) {
  if (!story.summary) return `More from ${story.sourceName || 'the source'}.`;
...
Summary: ${story.summary}${avoid}`;
  ...
  // The prompt above says "always write a body — never leave it empty,
  // even for a thin source", but that's still just an instruction: seen
  // in production as valid, cleanly-parsed JSON with a real headline and
  // body: null anyway, for no error parseJson() (or anything else) could
  // catch. One retry recovers most of these -- a bad response from a
  // non-deterministic model rarely repeats twice in a row.
  if (!hasNonEmptyBody(result) || !hasUsableHeadlineParts(result)) {
    result = await callAndParse();
  }

  // Still no body after the retry: fall back to the source's own
  // summary rather than ship a slide with no text on it at all. A
  // missing/unusable headline isn't papered over the same way -- there's
  // no safe way to fabricate one without risking an invented "fact" --
  // so that still throws, same never-silently-broken posture as before.
  if (!hasNonEmptyBody(result)) {
    result = { ...result, body: fallbackBody(story) };
  }
  if (!hasUsableHeadlineParts(result)) {
    throw new Error('Claude returned no usable headline for this story, even after a retry.');
  }
...
For a headline or body change, respond with JSON only:
{"action":"update","headline_parts":[...],"body":"...","note":"what you changed"}

For an image change, look at whether the message contains a URL:
- If it does, respond with JSON only:
  {"action":"image_url","url":"...","note":"what you changed"}
- If it doesn't (e.g. "use a different photo of Rodri", "find one without
  the crowd in it"), respond with JSON only:
  {"action":"image_search","query":"...","note":"what you're searching for"}
  The query must be ONLY the player/club names to search for — two or
  three words, the way someone would type it into a search box, never a
  full sentence and never a fee, score, or other non-name detail.
