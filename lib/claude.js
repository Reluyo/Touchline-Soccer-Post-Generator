...
{"translations":[{"index":0,"title":"...","summary":"..."}]}`;
  ...
  try {
    ...
    // If translation fails, return items untranslated rather than crashing
    console.warn(`[translateCandidates] Parse error, returning untranslated: ${err.message.slice(0, 100)}`);
    return items;
  }

  const byIndex = new Map(result.translations.map((t) => [t.index, t]));
