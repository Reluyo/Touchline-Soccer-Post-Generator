...
//
// Retries once, but only on a FAST-failing rate limit or server error --
// never on the 55s timeout below. A request that hangs long enough to hit
// that timeout has already spent nearly this whole route's 60s budget
// (see app/api/image/route.js), leaving no room for a second attempt; a
// 429/5xx response, by contrast, typically arrives in well under a
// second and leaves plenty of room.
async function requestImage(prompt, { retried = false } = {}) {
  ...
  if (!response.ok) {
    ...
    if (!retried && RETRYABLE_STATUS.has(response.status)) {
      await new Promise((resolve) => setTimeout(resolve, 500));
      return requestImage(prompt, { retried: true });
    }
...
export async function generateSlideImage({ role, context }) {
  if (!process.env.OPENAI_API_KEY) {
    // Fails loud and immediate rather than sending `Bearer undefined`
    // and waiting on a 401 round-trip for the same answer -- this is
    // the last fallback in the chain (after feed photo, then web
    // search), so unlike imageSearch.js there's nothing further to
    // silently fall through to.
    throw new Error('OPENAI_API_KEY is not set -- image generation is unavailable.');
  }
