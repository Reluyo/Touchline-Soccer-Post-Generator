...
> **Scope note:** this audit was performed against commit `1ea49bb`
> ("Switch Claude model to Sonnet 5"), the state of the app *before* the
> SixYardBox rebrand, the auto-ranking → manual News/Results picker
> rework, AI image generation, and the Brave Search image fallback landed
> on `main`. Findings about the old `/api/rank` auto-ranking flow (H-4,
> M-2, M-7) and several "harden JSON parsing" items (H-3, part of H-1) may
> already be addressed by later commits — see `main`'s commit history and
> `CLAUDE.md` for what's shipped since. Treat this as a snapshot of a
> known-old version, not a review of the current app; re-audit before
> acting on anything here as if it were current.
...
failure simulation, and a phased remediation plan follow, as they applied
at that commit.
